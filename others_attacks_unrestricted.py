...
def stadv_attack(im_tensor, ref_image=None, model=None, metric_range=100, device='cpu', lower_better=False, iters = 3, lr = 2.5):
    ALPHA = 50
    BETA = 1e-5
    input_prey = im_tensor.clone().to(device)
    flows = np.zeros((1,2,) + im_tensor.size()[2:])
    #print(flows.shape)
    flows = torch.from_numpy(flows)
    flows = torch.autograd.Variable(flows).to(device)
    flows.requires_grad = True
    opt = torch.optim.LBFGS([flows], lr=lr, max_iter=1, max_eval=50)
    ...
    x_adv = flow_st(im_tensor.clone().to(device), flows, device)
